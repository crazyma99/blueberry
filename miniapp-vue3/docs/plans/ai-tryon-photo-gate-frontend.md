...
| 2 | **埋点是否本期做** | 建议本期只做「端口 + fail-soft 调用」，不接第三方 SDK（**待主人拍板**）|
| 3 | ~~弹层形态~~ → **已定（主人 2026-09-23）：直接使用底部弹层**，复用既有 `ui/BasePopup` 门面并**接 Tokens** ✅（已实现于 `components/QualityRejectSheet`）|
| 4 | ~~未知码文案~~ → **已定（主人 2026-09-23）：兜底文案＝「换一张照片试试吧」** ✅（已落地 `PHOTO_GATE_COPY.unknown`，本地映射为准）|
...
## 8. 端侧 VK 预检严谨度评估（主人 2026-09-23 指示：「既然后端有了质量拦截，前端是不是不用拦那么死？你来评估」）

**现状**（`src/domain/photo-check.ts`，旧端 `utils/photoCheck.uts` 逐字移植；文件头注释明确「阈值与文案逐字保留——**改动需产品确认**」）：

| 端侧检查 | 现值 | 后端 4002 是否有对应 | 评估 |
|---|---|---|---|
| 分辨率（最短边） | `< 480` ⇒ 拦 | ❌ 无 | **保留**（零成本、判定确定；后端不看，用户能立即得到明确指引） |
| 模糊（拉普拉斯方差） | `< 100` ⇒ 拦 | ❌ 无 | **保留**（同上） |
| 无人脸 | `faceCount = 0` ⇒ 拦 | ✅ `no_face` | 保留（与后端一致，快速反馈） |
| 多人脸 | `> 1` ⇒ 拦 | ✅ `multi_face` | 保留（同上） |
| 人脸占比下限 | `< 0.02` ⇒ 拦 | ✅ `face_too_small`（后端阈值 **< 10%**） | **建议提到 `0.08`**：与后端同向但留 2pp 余量（VK 面积口径不如百度 V3 稳），明显过小的照片即时被拦、省一次 0.3–1s 云端往返 |
| 人脸占比上限 | `> 0.65` ⇒ 拦 | ❌ **后端无此规则** | ⭐ **建议移除硬拦**：特写/大头自拍会被端侧误杀，而后端判合格 ⇒ 这是当前**最该松绑**的一条 |
| 侧脸/角度 | 端侧不做 | ✅ `side_face` | ✅ 现状正确（端侧拿不到可靠角度 ⇒ 交后端） |
| fail-open | VK 不可用/测量失败 ⇒ 跳过 | ✅ 后端 fail-open | ✅ 现状正确（两层都 fail-open，不误杀） |

**结论**：后端的质量拦截**不能替代**端侧的两条「零成本确定性检查」（分辨率/模糊），它们的价值是**即时反馈 + 省云端调用**；
真正「拦得过死」的是**人脸占比上限 `0.65`**（后端没有这条规则）⇒ **建议移除或降级为提示**；
下限 `0.02` 建议提到 `0.08` 与后端 `10%` 同向对齐（留测量余量）；
**侧脸交给后端**（端侧不做）是正确取舍，应保持。

**改动清单（因代码注释要求产品确认 ⇒ 已列入待主人拍板，本轮未擅自改）**：
1. 删除 `PHOTO_FACE_AREA_MAX`（`> 0.65` 硬拦）与其文案；
