...
| 27 | 2026-09-23 | **v1.0.53** | `[g76cf3b0] 后端 4002 照片质量拦截前端对接：底部弹层（正反例对比／Tokens）＋4 码与未知码兜底＋端侧拦截统一 UX（删脸占比上限）＋埋点 ai_tryon_quality_reject` | ✅ **成功** | 主人目标：新分支 `feat/backend-tryon-photo-gate` 对接后端 staging 的 4002；四项指示落地（素材压缩 857KB→≈159KB 且**只在微信包**／底部弹层复用 `BasePopup`＋Tokens／未知码文案「换一张照片试试吧」／端侧 VK 评估）。**按调研结论与主人拍板实施的端侧改动**：删除「脸占比上限 0.65」硬拦（后端无上限、纯误杀）、**下限 0.02 保持**（量纲更正：端侧面积比 vs 后端单轴线性比）、端侧拦截**改走同一弹层**（保留端侧原话）、拦截后**清空旧照片**、新增**埋点端口**（fail-soft）并在两条路径上报。**后端前置已核实**：staging 部署二进制 VCS 指纹＝**`d444e98`**（＝staging tip）＋ `tryon_filter.enabled: true`。**独立 CR（两轮）**：首轮 🔴 两条（弹层缺卡片面／安全区写进 `padding` 简写）已修并产物实证；假绿 m2/m7/m9 已闭合；本轮 CR 结论＝**无 🔴、可发**，7 条 🟡（端侧 0.02 仍比后端严 2×／256px 降采样归因／AI 推荐页未同轮统一／埋点需后台登记事件／映射依赖逐字文案／3 处假绿待并入 probe／文档与注释收口）。CLI 实测 `TOTAL 1,884,236 B`（我的口径主包 1,469KB）。**待主人拍板**：⒜端侧下限 `0.02→0.01` 或改单轴线性；⒝AI 推荐页是否同轮统一；⒞微信后台需登记自定义分析事件 `ai_tryon_quality_reject`（字段 `check_code`/`source`），否则埋点静默 0 数据。 |
| 26 | 2026-09-23 | **v1.0.52** | `[g] 修「试衣 loading 进度不自动更新」：有服务端起始时间时 computed 失去响应式依赖被缓存 ⇒ 无条件读取内核每秒 tick 建立依赖` | ✅ **成功** | 主人报「推荐 loading 进度会自动更新，但试衣 loading 不会」。**根因（取证）**：内核 `ai-result-flow.ts:139-141` **本就每秒** emit `elapsedSeconds`；而 `use-fake-progress` 的 `progressPercent` 在 `startedAtMs` 有效时**不再读 tick** ⇒ computed **无依赖、被 Vue 缓存、永不重算**。试衣页几乎总能取到服务端 `created_at`（走墙钟分支）⇒ 冻住；推荐页新开一次 `resumed===0`（走回落分支）⇒ 每秒刷新——与主人观察完全一致。**修法**＝`progressPercent` 内无条件 `void source.elapsedSeconds.value;`；**守卫**＝`t60` 新增「假定时器推进墙钟 + tick 递增 ⇒ 25%→75%」用例（去掉该行即红）。`TOTAL 1.6 MB (1,711,361 B)`；`vitest` **530 passed／3 skipped**、`vue-tsc` 0、微信构建 exit 0。 |
| 25 | 2026-09-23 | **v1.0.51** | `[g] 修「等待中圆环不会自己推进」：门面自补间(1/100ms=10%/s)＋speed0（不再依赖 wot 内部 canvas 定时器）；CR 收口：upx2px 走裸 uni、删重复 speed、失败终态唯一出口、album-save 同根因修复` | ✅ **成功** | 主人报「等待过程中圆环的进度不会自己推进」⇒ 读 wot 源码定性：`wd-circle` 自带动画依赖**内部定时器 + canvas 节点查询**（`getContext()` 在 MP-WEIXIN 仅查到节点才 resolve）⇒ 真机易停在初始值。修法＝门面 `BaseCircle` **自补间**（`step=1`/`tickMs=100`＝10%/s，与伪进度同速）+ `speed` 默认 0；`GenerationProgress` 删 `:speed="60"`（防双跑）。**独立 CR 完整报告收口**：🔴1（t47 180s 用例被门面 `setInterval` 误抓）→ 改自调度 `setTimeout`；🟡1 `upx2px` 改裸 `uni`（`globalThis.uni` 在 mp-weixin 不存在；**同根因 `album-save` 真机 fail-closed 已修**）；🟡2 删重复 speed；🟡3 文案动画**如实口径**（opacity 入场真实生效、`scale` 原被 `<text>` 行内框忽略已加 `display:inline-block`、`:key` 在微信端不重挂载）；🟡5 失败终态唯一出口 `finishAsFailed()`（8 处）＋ t60 结构不变式守卫；🟡4 假绿点 M7/M8/M9 补测；🟡6 key 作用域化 `lm.aiRecommend.startedAt.v1:<shopId>:<filename>`＋走存储端口＋`line-height:1` 光学居中。`TOTAL 1.6 MB (1,711,331 B)`。**验证**：`vitest` **529 passed／3 skipped**、`vue-tsc` 0、微信+抖音构建 exit 0、变异 M7/M8/N3 全红。**待主人/设计确认**：环 200rpx ⇒ 等待面板高度 118→205px（可下调 160rpx）；后端 `created_at` 时区语义（若为 UTC 则解析早 8h ⇒ 被新鲜度窗口挡掉＝安全降级）。 | 
| 24 | 2026-09-23 | **v1.0.50** | `[g] AI等待页进度加 wot 环形进度动画（wd-circle 经门面 BaseCircle／绑定 tokens）＋进度文案动效；三段式保留；并修「离开再回来进度从 0 重来」` | ✅ **成功** | 主人目标：「AI 推荐与 AI 试衣 Loading 页进度文案没有动画 ⇒ 加 wot 动画与 Circle 组件并绑定 Tokens」，追加口径「**三段式保留、做增量**」＋追加需求「离开等待页再回来进度从 0 重来（体感丢进度）」。落地：门面 `ui/BaseCircle`（wot `wd-circle`）＋ 百分比文案动效（`:key` 重播 ＋ 当前步骤脉动）；token 绑定＝`colorAction`/`colorBorder` ＋ 新档位 `progressCircleSizeRpx=200`/`progressCircleStrokeRpx=12`；**续算**＝伪进度支持真实起始时间（试衣取服务端 `created_at`、推荐本地持久化＋180s 陈旧保护）。`TOTAL 1.6 MB (1,709,667 B)`。**独立 CR（子代理只读）临时结论：🔴 无**（三段式逐字保留/布局等价/token 闭环/抖音端不参与构建/`canvas type=2d` 同层渲染风险可接受），给出 **3 处低成本收尾（不阻塞）**：⒜推荐页失败/异常路径缺 `clearRecommendStartedAt()`（本人 commit 称「三处清理」与实现不符）；⒝无时区 `created_at` 按本机时区解析 ⇒ 跨时区会把已等待时间放大（过去方向无守卫）⇒ 建议补 `+08:00` 或加新鲜度窗口；⒞推荐页起始时间用裸 `uni.*StorageSync` 且 key 未按 op 作用域（建议走 `platform/uni/storage` 端口 + `lm.*.v1` 命名并带 shopId+filename）；另指出 **`t60` 的「页面接线源码守卫」是假绿点**（5 组变异全绿）⇒ 建议把「读/清/陈旧判定」抽纯函数做行为测试。**真机清单**：`device-acceptance-checklist.md` **§10（7 条）**，含 canvas 层级与基础库版本取证。 |
| 23 | 2026-09-23 | **v1.0.49** | `[g] 弹层收口：显示期隐藏自定义 Tab 栏 ＋ 关 root-portal/z-index 2000 ＋ 安全区后置 ＋ 抖音箱体重置；CR 2🔴＋9🟡 全处置` | ✅ **成功** | 主人报「v1.0.48 弹层仍在 Tab 栏下面被盖」＋「可通过 Web Search 找方案」。**查证结论（生态公认，见 [vant4 自定义 tabBar 遮挡弹层解法](https://jishuzhan.net/article/2064619043788763137)）**：**自定义 tabBar 位于独立元素树，页面内 `z-index` 压不过它、`wx.hideTabBar()` 对它也不生效** ⇒ 正解＝**弹层显示期间隐藏 Tab 栏**。落地：⒜`custom-tab-bar` 新增 `visible` 开关（根节点 `wx:if="{{visible}}"`）＋ `pageLifetimes.show` 兜底恢复；⒝门面 `BasePopup` 用 `getCurrentPages().getTabBar().setData({visible:false|true})` 在弹层显示/卸载时协同（fail-soft，非 tab 页静默）；⒞底部弹层同时 `:root-portal="false"`（回到页面内层叠，避免与运行时注入的 Tab 栏分属不同层）＋ `:z-index="2000"`。**独立 CR 处置**：🔴1 安全区 `padding-bottom` 原写在 `padding:` 简写之前＝死代码 ⇒ 后置（基数 44rpx）并产物复核；🔴2 抖音 `.is-bottom` 箱体未重置 `max-width:80%`／`padding:32rpx` ⇒ 补 `max-width:none;padding:0`；🟡3 删死 props（`round`/`safeAreaInsetBottom`）＋注释据实；🟡5 `border: $color-popup-card`；🟡6 孤儿 `@keyframes cardPopIn` 已删；🟡8 新增「已提交产物 == 重生成」守卫；🟡4 补 5 条守卫（LoginPopup 渲染级、token 面色/圆角/安全区、抖音 `is-bottom`、无死 API 负断言、Tab 栏协同渲染级）。**验证**：`vitest` **509 passed／3 skipped**、`vue-tsc` 0、**微信+抖音构建 exit 0**；产物实证 `wx:if="{{visible}}"` 已编译、门面 props 传参 `{position:"bottom","root-portal":!1,"z-index":2e3}`。**待真机**：弹层是否已完全可见（Tab 栏隐藏期）。 |
| 22 | 2026-09-22 | **v1.0.48**（45/46/47 为中间态） | `[g] 底部弹层收口：内容溢出修（box-sizing）／被 Tab 栏压着修（弹层 1200、Tab 栏 900）／面色改卡片 SCSS token（两端同源）／抖音自绘分支补底部形态；独立 CR 3 项全处置` | ✅ **成功** | 主人两项指示：⑥「保持业务逻辑不变 ⇒ 样式改 wot 底部 Popup（position=bottom）、Token 跟随主题色」＋ ⑦「v1.0.45 发现两个 UI 异常：Popup 被 Tab 栏压着、弹窗内容宽度大于弹窗」。处置：⒜**溢出**＝卡片 `width:100%` 未配 `box-sizing:border-box`（content-box 下 100%+左右 padding）⇒ 已加 `box-sizing:border-box`；⒝**层级**＝Tab 栏 `z-index:999` 压住弹层 ⇒ 弹层显式 `:z-index="1200"` ＋ **Tab 栏下调 900**（Tab 栏只需高于页面内容）；⒞**面色/圆角/安全区改由卡片承载**（`background: $color-popup-card`、上圆角 `#{$popup-radius-rpx*2}rpx`、`env(safe-area-inset-bottom)`）——**不用 CSS 变量**（抖音 TTSS 不支持，CR 🔴3 实证）⇒ **两端同源同值**；⒟门面 `BasePopup` 自绘分支补 `position=bottom`（底对齐+通栏+交给卡片出面）；⒠**独立 CR 三条**：红掉的 t57 断言（1001→1200）已修；⚠️**更正（CR 🟡4 复核）**：本行原称「token 假绿守卫已改真」**与当时代码不符**（`grep colorPopupCard tests/` 零命中）⇒ token 守卫是在 **v1.0.49** 才真正补上；跨端面问题已按 CR 建议②收口。**验证**：`vitest 503 passed／3 skipped`、`vue-tsc` 0、**微信+抖音构建 exit 0**；产物双端实证卡片＝`box-sizing:border-box;width:100%;background:#262626;border-radius:48rpx 48rpx 0 0`＋安全区；清缓存→关工具→`cli open` 重开→构建→上传全流程走完。**待真机**：底部弹层是否高于 Tab 栏、内容是否与屏宽对齐（左右各 48rpx）。 |
