...
| 18 | 2026-09-22 | **v1.0.41** | `[g…] 弹窗字体与主题绑定 token（fontFamily/卡片色/圆角）＋输入框与 placeholder 显式字体；门层层级 1001 与透明面` | ✅ **成功** | 主人反馈「字体问题还是存在、弹窗好像没变化、**风格和主题请绑定 token**」⇒ ①**字体族进 token 单一事实源**：`tokens/source.json` 新增 `primitive.fontFamilyBody/Serif`（生成器仅放行 `fontFamily*`，其余键仍严格白名单；新增回归 `t58`），弹窗**容器与 `.app-input-field` 显式 `font-family: $font-family-body`**（不再依赖全局类继承——弹窗被 `root-portal` 摘出 page 子树、且微信基座对 `wx-input` 硬编码字体），placeholder 内联 style 取自 `tokens.primitive.fontFamilyBody`；②**主题绑定 token**：新增 `semantic.colorPopupCard(#262626)`／`component.popupCardRadiusRpx(48)`，两弹窗样式按 theme.scss **值→变量机械化替换**（Profile 11 处／Login 9 处，值等价 ⇒ 零观感变化）；③产物实证：`ProfilePopup.wxss` 的 `.profile-card` 与 `.app-input-field`、`LoginPopup.wxss` 的 `.login-card` 均编译出 `font-family:HarmonyOS-Sans-SC`，`generated/tokens.js` 为字体唯一来源。`TOTAL 1.6 MB (1,696,652 B)`。token digest **`0ece3a71…`**。⚠️ **待主人核实**：若**全站**文字都像系统字体（不只弹窗），则根因是 **HarmonyOS woff 未加载**（微信后台 `downloadFile` 合法域名是否含 `lanmeiimgstore-1311468332.cos.ap-shanghai.myqcloud.com`；该域名已知为**抖音**侧已配、微信侧无记录；woff 本体本地实测 HTTP 200／font/woff／1,298,512 B） |
