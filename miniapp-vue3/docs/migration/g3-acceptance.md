...
| 3 | `P3-20` 页级场景测试 ＋ **T9b 独立 CR** | 🟡（**CR 已完成**） | 内核/静态层已覆盖（`t44` 连点·复用·失败不重发·分类；`t45` 禁自建轮询·分数口径·`#ifdef` 单块；`t46` 渲染与脏数据）；✅ **T9b 独立 CR 已完成（结论：无 🔴、可收口）**，其 6 条 🟡 已全清（含「入口页无代次守卫」的台账更正、入口页 `onHide` 清续跑标记、`deviations #9/#10` 登记内核更严与 `finalScore` 字符串口径、`t45` 补入口页断言）；**仅剩页级场景补测（401/4001/超时/弱网）** |
