...
| P4-12 平台/桥/generated 例外清单与扫描规则 | **本轮完成**：`scripts/scan-platform-usage.mjs`（敏感集＝`wx.*`/`tt.*`/`uni.request·uploadFile·downloadFile·login·authorize·openSetting·getImageInfo·chooseImage·saveImageToPhotosAlbum·getEnterOptionsSync·requestSubscribeMessage·requestPayment`；允许目录 `src/{platform,ui,generated}/**`；**UI 类 API 与注释提及不算违规**；例外须**显式登记＋理由**）＋`tests/pipeline/scan-platform-usage.spec.ts` 五例（含**真实仓库 0 违规**与「防登记幽灵例外」）；**扫描只对源码、不对第三方 bundle 盲 grep** | 🟡 **源码侧规则完成**；`配置/编译侧`待办（CR 指出 phases P4-12 含「配置/编译/代码」三侧，本批只覆盖源码 ts/vue） |
