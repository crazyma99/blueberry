...
| P4-12 平台/桥/generated 例外清单与扫描规则 | **本轮完成**：`scripts/scan-platform-usage.mjs`（敏感集＝`wx.*`/`tt.*`/`uni.request·uploadFile·downloadFile·login·authorize·openSetting·getImageInfo·chooseImage·saveImageToPhotosAlbum·getEnterOptionsSync·requestSubscribeMessage·requestPayment`；允许目录 `src/{platform,ui,generated}/**`；**UI 类 API 与注释提及不算违规**；例外须**显式登记＋理由**）＋`tests/pipeline/scan-platform-usage.spec.ts` 五例（含**真实仓库 0 违规**与「防登记幽灵例外」）；**扫描只对源码、不对第三方 bundle 盲 grep** | ✅ **源码侧＋配置侧均覆盖**（2026-09-17 补）：源码侧＝敏感 API／四类绕过（别名·解构·globalThis·方括号）；**配置侧**＝`src/pages.json`／`manifest.json`／`vite.config.*` 的「`#ifdef` 宏白名单＋配对校验」与「**`condition` 不得用于平台裁剪**（P4-07）」，实跑真实配置 **0 违规**（pages.json 11 个 `MP-WEIXIN` 全部登记且配对）。⚠️ **CI workflow 未纳入本规则**（由 workflow 自身「零 secret＋发布 job 硬停」口径约束） |
