...
describe("domain/photo-check（T8 S3b 纯规则）", () => {
  it("阈值常量与旧端逐字一致（480 / 100 / 0.02 / 0.65）", () => {
    expect([PHOTO_MIN_SIDE, PHOTO_BLUR_THRESHOLD, PHOTO_FACE_AREA_MIN, PHOTO_FACE_AREA_MAX]).toEqual([480, 100, 0.02, 0.65]);
  });
  ...
  it("⭐短路顺序与文案逐字：分辨率 → 模糊 → 未检出人脸 → 多张人脸 → 占比过小 → 占比过大", () => {
    ...
    expect(tooBig).toEqual({ ok: false, reason: "人脸占照片比例过大，请适当拉远距离后拍摄" });
  });
  ...
  it("variance=null（未测/无 canvas）与 faceCount=null（VK 不可用）→ 跳过对应检查，放行", () => {
    ...
    expect(evaluatePhotoCheck({ width: 800, height: 800, variance: 500, faceCount: 1, faceArea: PHOTO_FACE_AREA_MAX }).ok).toBe(true);
  });
});
