...
- sync-template.sh：将模板的公共代码（pages、utils、components 等）同步到目标仓库，避免手工维护漂移。
...
- [scripts/sync-template.sh:80-81](file://scripts/sync-template.sh#L80-L81)
...
- 功能概述
  ...
  - 不覆盖 package.json、project.config.json、manifest.json、pages.json、src/static、profiles/
- 关键参数
  - --repo <target-repo>：目标仓库路径
  - -h/--help：显示帮助
- 使用场景
...
- [scripts/new-miniapp-project.sh:83-83](file://scripts/new-miniapp-project.sh#L83-L83)
- [scripts/create-profile.sh:63-63](file://scripts/create-profile.sh#L63-L63)
- [scripts/release-miniapp.sh:14-14](file://scripts/release-miniapp.sh#L14-L14)
...
## 结论
本脚本体系通过“模板 + 多 Profile”的方式，实现了多小程序项目的标准化孵化与一键构建、校验、发布。建议在日常工作中优先使用 build-miniapp.sh 作为统一入口，并结合 sync-template.sh 与 apply-profile.sh 确保配置与模板的一致性；在发布前务必执行 verify-miniapp.sh 与 release-miniapp.sh 的导入路径提示，确保产物符合预期。
