...
## 更新摘要
**变更内容**
- 更新了生产环境API基础URL从'https://lanmei66.cloud'迁移至'https://crazyma99.xyz'
- 更新了配置文件中的默认测试环境URL
- 保持了构建期注入机制的完整性
...
- 关键行为
  - **已更新** 默认 baseURL 为 'https://crazyma99.xyz'（原为 'https://lanmei66.cloud'），默认 timeout 为 15000 毫秒
  - 该默认值会被构建期注入覆盖
- 运行时特性
...
### 开发环境与生产环境的配置差异
- 开发环境
  - **已更新** 默认 baseURL 在 config.uts 中现为 'https://crazyma99.xyz'（原为 'https://lanmei66.cloud'），便于本地联调
...
- 默认值
  - **已更新** baseURL: 'https://crazyma99.xyz'（构建期会被覆盖）
  - timeout: 15000（毫秒）
- 环境变量
...
- 常见问题
  ...
  - **已更新** 基础 URL 错误：确认 profile.env 中 API_BASE_URL 是否正确，以及是否成功注入到新的 'https://crazyma99.xyz' 域名
- 排查步骤
...
**已更新** 生产环境URL已从 'https://lanmei66.cloud' 迁移至 'https://crazyma99.xyz'，确保了服务的稳定性和安全性。
...
### 配置项与默认值对照表
- HttpConfig.baseURL
  - **已更新** 默认值：'https://crazyma99.xyz'（构建期注入后覆盖）
...
- 安全考虑
  ...
  - **已更新** 确保生产环境使用安全的 'https://crazyma99.xyz' 域名而非旧的 'https://lanmei66.cloud'