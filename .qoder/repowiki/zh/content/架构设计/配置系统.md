...
**最新更新**：HTTP请求基础URL配置已从测试环境'https://crazyma99.xyz'切换到新环境'https://lanmei66.cloud'，超时配置保持15000ms不变。
...
### HTTP请求基础URL配置更新
**最新更新**：HTTP请求基础URL配置已从测试环境'https://crazyma99.xyz'切换到新环境'https://lanmei66.cloud'。

- 配置位置
  - 主要配置文件：`src/utils/config.uts`中的`getHttpConfig()`函数
  - Profile配置：`profiles/*/project.env`中的`API_BASE_URL`字段
- 当前配置状态
  - baseURL：`https://lanmei66.cloud`（新环境）
...
D --> E["新环境<br/>https://lanmei66.cloud"]
```
...
- 当前环境配置
  - 基础URL：https://lanmei66.cloud（新环境）
  - 超时时间：15000ms
  - 应用代码：根据项目配置动态设置