...
## 2026-09-09 · 全站图片缩略优化（纯前端，服务端不变）

- Studio 客片网格 / 收藏页 / AI试衣记录：封面原图直出改为 600/400px WebP 缩略 + lazy-load（实测原图 8.9MB → 600webp 119KB，73 倍）；
- AI试衣页：模板主图 750 缩略、缩略图条 200 缩略 + 懒加载；
- 客片详情页：渐进加载——首图原图、其余 750 缩略 + 懒加载，预加载口径与渲染一致；
- 复用 cosThumb（仅处理 lanmei66.cloud 域名，其他来源原样返回）。

## 2026-09-09 · AI试衣模板缩略图快速选择（纯前端）

- 试衣页主图下方新增缩略图横滑条：展示当前相册内全部模板缩略图，当前选中项金色高亮描边；
- 点击缩略图直达对应模板（主 swiper 经 :current 联动）；保留左右滑动连续浏览；
- 缩略图条可折叠（点击箭头收起/展开），默认展开，位于主图下方不遮挡主图；沿用 design token 体系。
